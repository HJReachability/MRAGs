...
\author{Mo Chen, Zhengyuan Zhou, Claire J, Tomlin%
...
\begin{abstract}
A multiplayer reach-avoid game is a differential game between two adversarial teams, each of $\N$ cooperative players playing on a compact domain with obstacles. The attacking team aims to send $\m$ of the $\N$ attackers to some target location, while the defending team aims to prevent that by capturing or indefinitely delaying the attackers from reaching the target. The analysis of this game plays an important role in collision avoidance, motion planning, aircraft control, among other applications involving cooperative agents. The optimal solution to this game is not only difficult to intuit, but also computationally intractable when $\N>1$ due to the high dimensionality of the joint state space of all players. In this paper, we present a graph theoretic maximum matching approach that ties together the pairwise outcomes based on the solutions of the case where $\N=1$, while providing guarantees on the performance of the defending team. We will also present two different approaches for determining pairwise outcomes. We will show that the 4-dimensional Hamilton-Jacobi-Isaacs approach allows for real-time updates to the maximum matching in special cases, and the ``path defense" approach is more scalable with the number of players in general and trades off optimality for complexity while maintaining defender performance guarantees.
\end{abstract}
